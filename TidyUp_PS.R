...
## Gaeun: please note that gender variable was already renamed in the original data
...
tbl_clean$gender4 <- recode(tbl_clean$gender, 
                            '[2]' = 'female', '[1]' = 'male', '[5]' = 'queer/nonconforming', .default = 'others')
## gaeun: recoded as "female, male, queer/nonconforming, others"
...
tbl_clean <- tbl_clean %>%
  mutate(gender4 = recode(gender, 
         ...
         edu7 = case_when(edu < 3 ~ 'no highschool',
                          ...
                          edu == 9 | edu == 14 ~ 'masters (MA&MBA)',
                          edu > 9 & edu < 14 ~ 'advanced degree'))
## if we want to make edu7 variable as factor, should run the line below
tbl_clean$edu7 <- factor(tbl_clean$edu7)
summary(tbl_clean)
...
tbl_clean %>% 
  filter(edu7 == 'high school graduate', gender4 == 'male') %>%
